<template>
  <div id="address">
    <div class="addressshow" v-if="addToAddress">
      <div class="head">
        <img src="../../../static/images/fanhui.png" @click="fanhuiuppage"/>
        地址选择
      </div>
      <div class="content dialog-content grid">
        <ul class="consignee-list" v-if="isdefaultHasData">

          <li class="address-item active"  v-for="(item, index) in addressinfo"
              @click="optaddress(item.id)">

            <div class="row tool">
              <div class="col col-20 select-event-handle fb fvc">
                <div class="iconfont check-icon" @click="ifAddPic(item.id)" :class="{addCheckPic:item.id==picIndex}"></div>
                默认地址
              </div>
              <div class="col col-5 delete fb fvc far" @click="delAddress(item.id)">删除</div>
            </div>
            <div class="consignee-content row">
              <div class="col col-20">
                <div class="row ">
                  <div class="col contact">收货人：{{ item.receiverName }}</div>
                  <div class="col mobile fb fvc far">{{ item.receiverMobile }}</div>
                </div>
                <div class="row detailAddr"><p class="addr" province="北京市">{{ item.receiver_state }}{{ item.receiverCity
                  }}{{ item.receiverDistrict }}{{ item.receiverAddress }}</p></div>
                <div class="idCard hide"><p class=" col">身份证号：<span></span></p></div>
              </div>
              <div class="col col-3 fb far fvc edit-check">
        ...
        <div class="noCollectGoods" v-if="!isdefaultHasData">
            您还没有添加过任何收货地址！<br> 最多添加5个地址
        </div>
      </div>
      <div class="newaddress" v-if="addresslength<5" @click="addAddress">
        新建收货地址
      </div>
    </div>


    <div class="addressadd" v-if="!addToAddress">
      <div class="head">
        <span class="iconfont icon-cuohao" @click="closeAddresspage"></span>
        添加收货地址
      </div>
      <div class="main wl-edit-consignee-dialog">
        <div class="grid consignee-form addBox">
          <div class="row">
            <div class="col col-7 form-label">姓名</div>
            <div class="col col-17"><input class="name" type="text" placeholder="请输入姓名" maxlength="20" v-model="userName"></div>
            <div class="col col-1"></div>
          </div>
          <div class="row">
            <div class="col col-7 form-label">手机</div>
            <div class="col col-17"><input class="mobile" type="tel" placeholder="请输入手机号码" maxlength="11" v-model="userPhone" ></div>
            <div class="col col-1"></div>
          </div>
          <div class="row">
            <div class="col col-7 form-label">省份</div>
            <div class="col col-17"><input type="hidden" class="community" placeholder="请定位区域">
              <select name="sel" id="cmbProvince" v-model="prov">
          ...
          <div class="row community-row fvc">
            <div class="col col-7 form-label">城市</div>
            <div class="col col-17">
              <select id="cmbCity" v-model="city">
          ...
          <div class="row community-row fvc">
            <div class="col col-7 form-label">县、区</div>
            <div class="col col-17">
              <select id="cmbArea" v-model="district" v-if="district">
                  <option v-for="option in districtArr" :value="option.name">{{ option.name }}</option>
              </select>
              <input id="cmbProvince_input" type="hidden"><input
              id="cmbCity_input" type="hidden"><input id="cmbArea_input" type="hidden"></div>
            <div class="col col-1"></div>
          </div>
          <div class="row">
            <div class="col col-7 form-label">详细地址</div>
            <div class="col col-17"><input class="addr" type="text" placeholder="请填写详细地址" maxlength="200" v-model="detailedAddress"></div>
            <div class="col col-1"></div>
          </div>
          <div class="row idCard_row" style="display: none;">
            <div class="col col-7 form-label">身份证号</div>
            <div class="col col-17"><input class="idCard" type="tel" placeholder="请填写身份证号" maxlength="200"></div>
            <div class="col col-1"></div>
          </div>
        </div>
        <div class="detail-address-text">注：地址填写尽量详细，如xx楼xx室；</div>
        <div class="button submit" @click="preservationAddress">保存</div>
      </div>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
          ...
          .then(res => {
            if(res.code==0){
              if(res.result.consignees.length==0){
                this.isdefaultHasData=false
              }else{
                this.isdefaultHasData=true
                that.addressinfo = res.result.consignees;
                that.addresslength = res.result.consignees.length;
                for(var i=0;i<res.result.consignees.length;i++){
                  if(res.result.consignees[i].getDef==1){
                    this.picIndex=res.result.consignees[i].id
                    return
                  }
                }
              }
      ...
      addAddress: function () {
        this.addToAddress=false
      },
      closeAddresspage: function () {
        this.addToAddress=true
      },
      //返回上一个路由
        ...
        if(this.userName!=""){
          if(this.userPhone!=""){
            if(this.detailedAddress!=""){
              var addrs='{"city":"'+this.city+'","district":"'+this.district+'","province":"'+this.prov+'","name":"'+this.detailedAddress+'","address":"'+this.detailedAddress+'"}'
              axios
                .post(
                  "saveAddress",
                  qs.stringify({
...
<style scoped>
  ...
  select{
    ...
    background-image: url(http://unesmall.b0.upaiyun.com/common/images/icon_dropdown.png);
  ...
  .wl-edit-consignee-dialog .addr, .wl-edit-consignee-dialog .community, .wl-edit-consignee-dialog .idCard, .wl-edit-consignee-dialog .mobile, .wl-edit-consignee-dialog .name{
    width: 100%;
  }
  .grid .row .col.col-17{
    flex: 68;
  ...
  .address-item .addr, .address-item .idCard {
    font-size: 13px;
  }
  ...
  .address-item .consignee-content .idCard, .address-item .consignee-content .row {
    padding: 3px;
    border-bottom-width: 0;
  }

  .address-item .idCard, .address-item .row {
    position: relative;
    background: #fff;
    padding: 10px 4%;
  ...
  .address-item .idCard:last-child, .address-item .row:last-child {
    border-bottom-width: 0;
  }

  .address-item .delete:before {
    content: '';
    background-image: url(http://unesmall.b0.upaiyun.com/common/images/addr_list_delete.png);
    width: 24px;
    height: 24px;
...
.address-item .check-icon:after {
  ...
  .addCheckPic{
    background: url("../../../static/images/gouhao.png") no-repeat;
  }
.iconfont.check-icon:after {
    content: "";
  }
  .fb, .grid .row {
    display: flex;
