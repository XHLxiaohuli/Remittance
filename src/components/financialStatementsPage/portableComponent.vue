<template>
    <div>
        <!--有数据start-->
        <div v-if="isCanBePresented">
            <div class="grid  presentList wl-nexter">
                <ul>
                    <li v-for="(item,index) in portabData">
                        <div class="row row_border">
                            ...
                            <div class="col col-5 fb far fvc">
                                <span class="status_img">
                                    <img src="http://unesmall.b0.upaiyun.com/common/images/icon_withdrawal.png" alt="">
                                </span>
                            </div>
                        </div>
...
export default{
    ...
    methods:{
        ...
        initialCanBePresented(){
            if(this.myValue2.length>0){
                this.isCanBePresented=true
                for(var i=0;i<this.myValue2.length;i++){
                    this.myValue2[i]["time"]=this.timestampToTime(this.myValue2[i].dateCreated)
                }
                this.portabData=this.myValue2
            }else{
